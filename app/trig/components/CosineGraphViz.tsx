...
export default function CosineGraphViz() {
  ...
  const points = useMemo(() => {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      const y = amplitude * Math.cos(frequency * x + phase);
      pts.push({ x, y });
    }
    return pts;
  ...
  const featurePoints = useMemo(() => {
    ...
    // zeros: cos(bx + c) = 0 => bx + c = pi/2 + n*pi
    for (let n = -10; n <= 20; n++) {
      const x = (Math.PI / 2 + n * Math.PI - phase) / frequency;
      if (x >= X_MIN && x <= X_MAX) {
        fps.push({ x, y: 0, type: "zero" });
      }
    }

    // maxima: cos(bx + c) = 1 => bx + c = 2n*pi
    for (let n = -10; n <= 10; n++) {
      const x = (2 * n * Math.PI - phase) / frequency;
      if (x >= X_MIN && x <= X_MAX) {
        fps.push({ x, y: amplitude, type: "max" });
      }
    }

    // minima: cos(bx + c) = -1 => bx + c = (2n+1)*pi
    for (let n = -10; n <= 10; n++) {
      const x = ((2 * n + 1) * Math.PI - phase) / frequency;
      if (x >= X_MIN && x <= X_MAX) {
    ...
    <div className="space-y-6">
      ...
      <div className="overflow-x-auto">
        ...
        >
          ...
          <line x1={PADDING} y1={SVG_H / 2} x2={SVG_W - PADDING} y2={SVG_H / 2} stroke="#94a3b8" strokeWidth={1} />
          <line x1={toSVGX(0)} y1={10} x2={toSVGX(0)} y2={SVG_H - 10} stroke="#94a3b8" strokeWidth={1} />
          ...
          {featurePoints.map((fp, i) => {
            ...
            const color = fp.type === "zero" ? "#64748b" : fp.type === "max" ? "#ef4444" : "#22c55e";
            return (
              <circle key={`fp-${i}`} cx={cx} cy={cy} r={3} fill={color} stroke="white" strokeWidth={1} />
            );
          })}
        </svg>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 text-sm">
        ...
        <div className="bg-green-50 dark:bg-green-900/30 p-3 rounded-xl text-center">
          ...
          <K tex={periodTex} />
          {period !== Infinity && (
            <div className="text-[10px] text-slate-500 mt-1">({period.toFixed(2)})</div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div>
          <label className="flex items-center justify-between text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">
            <span>振幅 a</span>
            <span className="font-mono text-purple-600">{amplitude}</span>
          </label>
        ...
        <div>
          <label className="flex items-center justify-between text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">
            <span>角周波数 b</span>
            <span className="font-mono text-purple-600">{frequency}</span>
          </label>
        ...
        <div>
          <label className="flex items-center justify-between text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">
            <span>位相 c</span>
            <span className="font-mono text-purple-600">{phase.toFixed(1)}</span>
          </label>
          <input type="range" min={-Math.PI} max={Math.PI} step={0.1} value={phase}
            onChange={(e) => setPhase(parseFloat(e.target.value))}
            className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-full appearance-none accent-purple-500 cursor-pointer" />
        </div>
      </div>

      <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl text-sm text-slate-600 dark:text-slate-400">
        <p className="font-bold mb-2">sin と cos の関係</p>
