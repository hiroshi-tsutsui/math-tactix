...
const LEVELS = [
  { id: 1, name: "変化の検出", desc: "グラフの傾き（微分係数）を 2.0 以上にしてください。", targetSlope: 2.0, type: 'diff' },
  { id: 2, name: "極値の特定", desc: "グラフの傾きがほぼ 0 になる点（頂点）を見つけてください。", targetSlope: 0.1, type: 'zero' },
  { id: 3, name: "領域の集積", desc: "グラフの下側の面積（積分）を 10.0 以上にしてください。", targetArea: 10.0, type: 'int' }
];
...
export default function CalculusPage() {
  ...
  const currentVal = f(t);
  const currentSlope = df(t);
  const currentArea = integral(t);
      ...
      <nav className="border-b border-slate-200 bg-white/80 backdrop-blur-md sticky top-0 z-50 h-16">
        <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm font-bold">微分・積分</span>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8">
          <div className="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden">
            <div className="p-6 bg-white border-b border-slate-200 flex justify-between">
              <h2 className="font-bold flex items-center gap-2"><Activity className="w-4 h-4 text-blue-600" /> 変化の可視化</h2>
              <div className="flex gap-6 text-xs font-mono">
                <div><span className="text-slate-400">傾き f'(x):</span> <span className="text-blue-600 font-bold">{currentSlope.toFixed(2)}</span></div>
                <div><span className="text-slate-400">面積 ∫:</span> <span className="text-blue-600 font-bold">{currentArea.toFixed(2)}</span></div>
              </div>
            </div>
            ...
            <div className="p-12">
               <div className="relative h-64 bg-white border border-slate-100 rounded-xl mb-8 flex items-end px-12">
                  ...
                  <svg className="w-full h-full overflow-visible">
                    ...
                    />
                    <circle cx={t * 400} cy={200 - (f(t) * 200)} r="6" fill="#3b82f6" />
                    {/* Tangent Line */}
                    <line 
                      x1={(t-0.1)*400} y1={200 - (f(t) * 200) + (currentSlope * 40)}
                      x2={(t+0.1)*400} y2={200 - (f(t) * 200) - (currentSlope * 40)}
              ...
              {showComplete && (
                <div className="p-6 bg-green-50 border-t border-green-100 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-bold text-green-800">目標達成！数学的本質を捉えました。</span>
                  </div>
                  <button onClick={handleNext} className="bg-slate-900 text-white px-6 py-2 rounded-lg font-bold text-sm">次へ進む</button>
                </div>
              )}
            </AnimatePresence>
          </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-slate-900 rounded-2xl p-8 text-white">
            <h3 className="text-[10px] font-bold text-blue-400 uppercase mb-2">現在のミッション</h3>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6">
            <h4 className="text-xs font-bold text-slate-400 uppercase mb-4">活動ログ</h4>
