const fs = require('fs');
const path = './app/math_i/numbers/page.tsx';
...
if (!content.includes('絶対値を含む不等式 (場合分け)')) {
    ...
    if (content.includes('id: "integer-solutions-inequality"')) {
        ...
        if (levelsMatch) {
            content = content.replace(levelsMatch[0], `const levels = [${levelsMatch[1]}, ${newLevel}];`);
        }
    }
}

fs.writeFileSync(path, content);
console.log('Updated page.tsx with Level 20');
